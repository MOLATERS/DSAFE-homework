...
void SetWeight(FILE* fp)//记录权值
{
    ...
    for(int i=0;i<=count-1;i++) N[i].weight=N[i].weight/total;
}
void Code()//进行哈夫曼编码
{
...
void Transfer(FILE* fp,FILE* fq,long long* size)//读取文件并转换为哈夫曼编码
{
    char ch;
    char p[16];
    unsigned char temp;
    ...
    *size=1;
    while(ch!=EOF)
    {
        for(int i=0;i<count;i++)if(ch==D[i].ch){strcpy(p,D[i].bit);break;}
...
void Recover(const HFM T[2*maxlength], FILE* fq,FILE* fg,long long *size)//将哈夫曼编码还原为文本文件
{
    unsigned char ch;
    int start=2*count-2;
    fseek(fq,0,SEEK_SET);
    while(fread(&ch,sizeof(unsigned char),1,fq))
    {
        for(int i=0;i<8;++i)
        {
            if(ch&128)start=T[start].rchild;
            else start=T[start].lchild;
            if (start<count)
            {
