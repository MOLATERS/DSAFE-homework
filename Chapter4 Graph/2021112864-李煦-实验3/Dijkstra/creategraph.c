...
void CreatMGraph(MTgraph*G)//邻接矩阵表示，数组表示法
{
    ...
    int w;//记录权值
    int i,j;
    FILE* fp=fopen("D:\\SOPHOMORE_1\\DSAFEcode\\Chapter4 Graph\\lab3\\Dijkstra\\graph.txt","r");//将图中结点信息保存在文件中
    FILE* fq=fopen("D:\\SOPHOMORE_1\\DSAFEcode\\Chapter4 Graph\\lab3\\Dijkstra\\data.txt","r");
